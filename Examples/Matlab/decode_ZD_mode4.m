%% decode_ZD_mode4.m
% 用于对MT_IMU产品的mode4数据进行解析(49bytes)
% ============================================================================
...
for i = 1:n
    ...
    [packet,count] = fread(fid,47,'uint8');
    fseek(fid,-47,'cof');
    [ck1_out,ck2_out] = double_checksum_func(packet, 45);

    if packet(3)==42 && ck1_out==packet(46) && ck2_out==packet(47)
        % count
